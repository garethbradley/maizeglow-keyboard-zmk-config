...
/**
 * @file Driver for mpr121 I2C-based GPIO driver.
 */
...
LOG_MODULE_REGISTER(gpio_mpr121);
...
static int read_registers(const struct device *dev, uint8_t reg, uint16_t *buf) {
    ...
    *buf = sys_get_le16(data);
    ...
    LOG_DBG("mpr121: read: reg[0x%X] = 0x%X, reg[0x%X] = 0x%X", reg, data[0], (reg + 1), data[1]);
...
static int write_registers(const struct device *dev, uint8_t reg, uint16_t value) {
    ...
    LOG_DBG("mpr121: write: reg[0x%X] = 0x%X, reg[0x%X] = 0x%X", reg, (value & 0xFF), (reg + 1),
            (value >> 8));
...
static int set_pin_direction(const struct device *dev, uint32_t pin, int flags) {
    struct mpr121_drv_data *const drv_data = (struct mpr121_drv_data *const)dev->data;
 ...
 *        registers, since the mpr121 only supports a pullup, and it can't be controlled.
 *
 * @param dev   The mpr121 device.
 * @param pin   The pin number
 * @param flags Flags of pin or port
...
static int mpr121_config(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags) {
    struct mpr121_drv_data *const drv_data = (struct mpr121_drv_data *const)dev->data;
...
static int mpr121_port_get_raw(const struct device *dev, uint32_t *value) {
    struct mpr121_drv_data *const drv_data = (struct mpr121_drv_data *const)dev->data;
...
static int mpr121_port_set_masked_raw(const struct device *dev, uint32_t mask, uint32_t value) {
    struct mpr121_drv_data *const drv_data = (struct mpr121_drv_data *const)dev->data;
...
static int mpr121_port_set_bits_raw(const struct device *dev, uint32_t mask) {
    return mpr121_port_set_masked_raw(dev, mask, mask);
}

static int mpr121_port_clear_bits_raw(const struct device *dev, uint32_t mask) {
    return mpr121_port_set_masked_raw(dev, mask, 0);
}

static int mpr121_port_toggle_bits(const struct device *dev, uint32_t mask) {
    struct mpr121_drv_data *const drv_data = (struct mpr121_drv_data *const)dev->data;
...
static int mpr121_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
                                           enum gpio_int_mode mode, enum gpio_int_trig trig) {
    return -ENOTSUP;
}

static const struct gpio_driver_api api_table = {
    .pin_configure = mpr121_config,
    .port_get_raw = mpr121_port_get_raw,
    .port_set_masked_raw = mpr121_port_set_masked_raw,
    .port_set_bits_raw = mpr121_port_set_bits_raw,
    .port_clear_bits_raw = mpr121_port_clear_bits_raw,
    .port_toggle_bits = mpr121_port_toggle_bits,
    .pin_interrupt_configure = mpr121_pin_interrupt_configure,
};

/**
 * @brief Initialisation function of mpr121
 *
 * @param dev Device struct
 * @return 0 if successful, failed otherwise.
 */
static int mpr121_init(const struct device *dev) {
    const struct mpr121_config *const config = dev->config;
    struct mpr121_drv_data *const drv_data = (struct mpr121_drv_data *const)dev->data;
...
#define MPR121_INIT(inst)                                                                         \
    static struct mpr121_config mpr121_##inst##_config = {                                       \
        .common = {.port_pin_mask = GPIO_PORT_PIN_MASK_FROM_DT_INST(inst)},                        \
        .i2c_bus = I2C_DT_SPEC_INST_GET(inst)};                                                    \
                                                                                                   \
    static struct mpr121_drv_data mpr121_##inst##_drvdata = {                                    \
        ...
        .reg_cache.output = 0xFFFF,                                                                \
                                                                                                   ...
                                                                                                   \
    DEVICE_DT_INST_DEFINE(inst, mpr121_init, NULL, &mpr121_##inst##_drvdata,                     \
                          &mpr121_##inst##_config, POST_KERNEL,                                   \
                          CONFIG_GPIO_MPR121_INIT_PRIORITY, &api_table);
